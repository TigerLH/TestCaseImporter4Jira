...
public class JiraUtil {
	...
	public List<String> getAllProject() throws Exception {
		...
		Iterator<BasicProject> projects = restClient.getProjectClient()
				.getAllProjects().get().iterator();
	 ...
	 * @throws Exception
	 */
	public void AddIssue(String projectName, TestCase testCase)
			throws Exception {
		...
		String sprntId = this.getFiledIdByName(CustomFieldsName.SPRINT
				.getCode());
		long priorityId = this.getPrioritieBykey(testCase.getPriorityName());
		String reporter = testCase.getReporter();

		IssueInputBuilder builder = new IssueInputBuilder(projectKey, caseType,
		...
		if (testCase.getLabels() != null) { // 允许标签为空
			builder.setFieldInput(new FieldInput(IssueFieldId.LABELS_FIELD,
					Arrays.asList(testCase.getLabels())));
		}
		...
		if (testCase.getCommit() != null) { // 允许备注为空
			client.addComment(actual.getCommentsUri(),
					Comment.valueOf(testCase.getCommit()));
		}
	}
	...
	private Long getPrioritieBykey(String key) throws Exception {
		Promise<Iterable<Priority>> priorotys = restClient.getMetadataClient()
				.getPriorities();
	...
	private String getFiledIdByName(String name) throws Exception {
		Iterator<Field> Fileds = restClient.getMetadataClient().getFields()
				.get().iterator();
